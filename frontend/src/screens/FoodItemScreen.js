...
import { FOOD_ITEM_CREATE_REVIEW_RESET } from "../constants/foodItemConstants";
...
const FoodItemScreen = ({ history, match }) => {
	...
	useEffect(() => {
		...
		if (!foodItem._id || foodItem._id !== match.params.id) {
			...
			dispatch({ type: FOOD_ITEM_CREATE_REVIEW_RESET });
		}
	}, [dispatch, match, successFoodItemReview, foodItem._id]);
					...
					<Row>
						...
						<Col md={3}>
							<ListGroup variant='flush'>
								...
								<ListGroup.Item>Price: Rs - {foodItem.price}</ListGroup.Item>
								<ListGroup.Item>
								...
								<ListGroup variant='flush'>
									<ListGroup.Item>
										<Row>
											<Col>Price:</Col>
											<Col>
												<strong> Rs - {foodItem.price}</strong>
											</Col>
										</Row>
									</ListGroup.Item>
